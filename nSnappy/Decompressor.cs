...
namespace NSnappy
{
	public class Decompressor
	{
		...
		public static int Decompress(Stream input, Stream output)
		{
			...
			return decompressor.RawUncompress(compressed, output);
		}

		private int RawUncompress(byte[] compressed, Stream output)
		{
			...
			for (;;)
			{
				...
				if ((c & 0x3) == CompressorTag.Literal)
				{
					...
					if (literalLength >= 61)
					{
						...
						literalLength = (int) ((ip.ToUInt32() & _wordmask[longLiteral]) + 1);
						ip += longLiteral;
					}
				...
				else
				{
					...
					int trailer = (int) (ip.ToUInt32() & _wordmask[entry >> 11]);
					...
					// bit 8).
					int copy_offset = entry & 0x700;
					if (!_output.AppendFromSelf(copy_offset + trailer, length))
					{
						return;
					}
					if ( !MAYBE_REFILL() )
...
//      (2) It properly aligns copy offset so we do not need a <<8
		...
		private static readonly uint[] _wordmask =
			{
